...
const Emergency = () => {
  ...
  const userFirstName = status === "authenticated" && data?.user?.name ? data.user.name.split(' ')[0] : '';
  ...
  const loadEmergencyData = async (userData) => {
    if (!userData?.user?.email) return;
    setLoading(true);
    try {
      const docRef = doc(db, 'emergencyData', userData.user.email);
  ...
  useEffect(() => {
    if (data) loadEmergencyData(data);
  }, [data]);
  ...
  const tableData = [
    ...
    ['RAHI - Support centre for women survivors of abuse', '(011) 26238466/ 26224042, 26227647'],
  ];
  ...
  return (
    <div>
      ...
      <div className='border shadow-sm border-gray-200 rounded-xl'>
        <a href={`tel:${emergencyData?.contact}`} className='cursor-pointer p-3 text-center w-full flex flex-row justify-between items-center gap-2 rounded-xl'>
        ...
        <hr />
        <a href={'tel:7827170170'} className='cursor-pointer p-3 text-center w-full flex flex-row justify-between items-center gap-2 rounded-xl'>
          <div className='flex flex-row justify-start items-center gap-2'>
        ...
        <hr />
        <a href={'tel:108'} className='cursor-pointer  p-3 text-center w-full flex flex-row justify-between items-center gap-2 rounded-xl'>
          <div className='flex flex-row justify-start items-center gap-2'>
              ...
              <TableBody>
                {tableData.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className='text-right' onClick={() => handleCopyToClipboard(item[1])}>
                      {item[1]}
                    </TableCell>
                  </TableRow>
                ))}
      ...
      <Link href="/emergency-data" className="cursor-pointer border shadow-sm border-gray-200 rounded-xl font-semibold px-2 py-3 text-center w-full flex flex-row justify-start items-center gap-2">
